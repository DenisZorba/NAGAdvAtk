from data import load_imagenet, remove_inaccurate_imgs
...
imgs, labels = load_imagenet(num_img=10, filter_label=None, 
                             path='../data', # replace with the path to the imagenet val set
                             )
...
solver_options = ['Print Solution = NO',
                  ...
                  'Outer Iteration Limit = 500', 
                  'Stop Tolerance 1 = 0.1',
                  ]


attacker = nag_atk(imgs, labels, model,
                   atk_target=3, # perturb imgs to class 3
                   mode=1, # 1 for targeted, 2 for untargeted (which ignores atk_target)
                   optimiser='ipopt', # ipopt for ssqp
                   p=2,
